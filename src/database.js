...
function saveDb() { fs.writeFileSync(DB_PATH, JSON.stringify(_db, null, 2), 'utf8'); }

function initDb() {
  fs.mkdirSync(path.dirname(DB_PATH), { recursive: true });
  if (fs.existsSync(DB_PATH)) {
    try { _db = JSON.parse(fs.readFileSync(DB_PATH, 'utf8')); } catch(e) { _db = null; }
  }
...
function seedData() {
    ...
    months.forEach((month, mi) => {
      if (Math.random() > 0.12) {
        ...
        _db.assessments.push({
          id:`a_${s.id}_${month}`, studentId:s.id, providerId:s.providerId,
          month, year:'תשפ״ו', source:'manual', createdAt:new Date().toISOString(),
          categories:{
  ...
  _db.systemLog = [
    {id:uuidv4(),type:'success',message:'KriahTrack initialized successfully',user_name:'System',created_at:new Date().toISOString()},
    {id:uuidv4(),type:'info',   message:'Demo data loaded — 12 students, 4 providers',user_name:'System',created_at:new Date().toISOString()},
  ];
  _db.auditLog = []; _db.ocrImports = [];
}

const MONTH_ORDER = {tishrei:1,cheshvan:2,kislev:3,tevet:4,shvat:5,adar:6,nisan:7,iyar:8,sivan:9,tamuz:10,av:11,elul:12};

function addAuditLog(action,entity,entityName,field,before,after) {
  _db.auditLog.unshift({id:uuidv4(),action,entity,entity_name:entityName,field:field||'',before_val:String(before||''),after_val:String(after||''),user_name:'Admin',created_at:new Date().toISOString()});
  if(_db.auditLog.length>500) _db.auditLog=_db.auditLog.slice(0,500);
  saveDb();
}
function addSystemLog(type,message,user) {
  _db.systemLog.unshift({id:uuidv4(),type,message,user_name:user||'Admin',created_at:new Date().toISOString()});
...
function createProvider(data) {
  ...
  _db.providers.push(p); addAuditLog('Add Provider','Provider',data.name,'—','—','Created'); addSystemLog('info',`Provider added: ${data.name}`); saveDb(); return p;
}
function updateProvider(id,data) {
  const idx=_db.providers.findIndex(p=>p.id===id); if(idx<0) return null;
  _db.providers[idx]={..._db.providers[idx],...data,id,updatedAt:new Date().toISOString()}; saveDb(); return _db.providers[idx];
}
...
function getStudentsByProvider(pid) { return _db.students.filter(s=>s.providerId===pid); }
function createStudent(data) {
  ...
  _db.students.push(s); addAuditLog('Add Student','Student',`${data.firstName} ${data.lastName}`,'—','—','Created'); addSystemLog('info',`Student added: ${data.firstName} ${data.lastName}`); saveDb(); return s;
}
function updateStudent(id,data) {
  const idx=_db.students.findIndex(s=>s.id===id); if(idx<0) return null;
  _db.students[idx]={..._db.students[idx],...data,id,updatedAt:new Date().toISOString()}; saveDb(); return _db.students[idx];
}
function deleteStudent(id) {
  const s=getStudentById(id); _db.students=_db.students.filter(x=>x.id!==id); _db.assessments=_db.assessments.filter(a=>a.studentId!==id);
  if(s) addAuditLog('Delete Student','Student',`${s.firstName} ${s.lastName}`,'—','Exists','Deleted'); saveDb();
}

function getAssessmentsByStudent(sid) { return _db.assessments.filter(a=>a.studentId===sid).sort((a,b)=>(MONTH_ORDER[a.month]||0)-(MONTH_ORDER[b.month]||0)); }
function getAssessmentsByProvider(pid) { return _db.assessments.filter(a=>a.providerId===pid); }
...
function upsertAssessment(data) {
  ...
  if(idx>=0) {
    ...
    addAuditLog('Update Assessment','Assessment',data.studentName||'Student','Month',data.month,'Updated'); saveDb(); return {id:_db.assessments[idx].id,updated:true};
  } else {
    ...
    _db.assessments.push(a); addAuditLog('Add Assessment','Assessment',data.studentName||'Student','Month','—',data.month); saveDb(); return {id:a.id,updated:false};
  }
}
function deleteAssessment(id) {
  const a=_db.assessments.find(x=>x.id===id); _db.assessments=_db.assessments.filter(x=>x.id!==id);
  if(a) addAuditLog('Delete Assessment','Assessment',a.studentId,'Month',a.month,'Deleted'); saveDb();
}
...
function saveOCRImport(data) {
  ...
  _db.ocrImports.unshift(imp); addSystemLog('success',`OCR import — ${data.imported} imported, ${data.skipped} skipped`); saveDb(); return imp.id;
}
function getAllOCRImports() { return _db.ocrImports; }
function exportAll() { return {exportedAt:new Date().toISOString(),providers:getAllProviders(),students:getAllStudents(),assessments:getAllAssessments()}; }
...
module.exports = {
  ...
  getAssessmentsByStudent,getAssessmentsByProvider,getAllAssessments,getAssessmentsByMonth,upsertAssessment,deleteAssessment,
